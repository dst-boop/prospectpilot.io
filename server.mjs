...
const prospectJobs=createProspectJobs({pool,providers:createProspectProviders({pdlKey:process.env.PDL_API_KEY||'',hunterKey:process.env.HUNTER_API_KEY||'',trestleKey:process.env.TRESTLE_API_KEY||'',anthropicKey:process.env.ANTHROPIC_API_KEY||''}),config:providerJobConfig(),dispatch:async()=>{const token=await applicationDefault().getAccessToken();const region=process.env.GOOGLE_CLOUD_REGION||'us-central1';const response=await fetch('https://run.googleapis.com/v2/projects/'+process.env.GOOGLE_CLOUD_PROJECT+'/locations/'+region+'/jobs/prospectpilot-research:run',{method:'POST',headers:{Authorization:'Bearer '+token.access_token,'Content-Type':'application/json'},body:'{}',signal:AbortSignal.timeout(10000)});await response.body?.cancel();return response.ok;}});
