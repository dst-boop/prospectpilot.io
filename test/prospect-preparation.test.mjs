...
async function fixture(fn){const db=new PGlite();const calls=[];let check=async c=>({email:c.email,domain:c.email.split('@')[1],status:'mx_present',checked_at:new Date().toISOString(),provider:'dns'});try{await db.exec(readFileSync(new URL('../migrations/008-prospect-workspace.sql',import.meta.url),'utf8'));await db.exec(readFileSync(new URL('../migrations/017-forget.sql',import.meta.url),'utf8'));const app=createProspectWorkspace({pool:{query:(...a)=>db.query(...a),connect:async()=>({query:(...a)=>db.query(...a),release(){}})},checkDomain:async c=>{calls.push(c);return check(c);}});await fn(app,calls,fn=>check=fn);}finally{await db.close();}}
