...
test('catalog refresh rolls back every batch on malformed input and rejects older amendments',async()=>{
 const db=new PGlite();try{
  ...
  const before=await catalogSummary(db);assert.equal(before.filings,1);assert.equal(before.plans,1);
  ...
  assert.deepEqual(await catalogSummary(db),before);
  const result=await importPlanRecords(db,[make(0,'a')]);assert.equal(result.older_filings_skipped,1);assert.deepEqual(await catalogSummary(db),before);
...
test('catalog summary migration backfills exact plan totals and dashboard reads avoid catalog scans',async()=>{
 const db=new PGlite();try{
  ...
  assert.equal(summary.plans,2);assert.equal(summary.filings,3);assert.equal(summary.latest_plan_year,2025);
 }finally{await db.close();}
});
