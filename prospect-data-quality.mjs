...
const hostname = value => /^(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z]{2,}$/i.test(value);
export function normalizeContact(raw, source) {
  ...
  if (rawEmail && (!contact.email || !hostname(contact.email.split('@')[1]) || /^\.|\.\.|\.@/.test(contact.email))) throw fail('Invalid email address.');
