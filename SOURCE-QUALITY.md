...
## Contact corrections and source decisions

Open a contact and expand **Correct contact details** to update professional fields with a review reason or evidence reference. The server checks the current record version, validates identifiers, preserves suppression and rejects identifiers already attached to another contact. Email/phone changes reset the corresponding status; name changes reset both. Historical check metadata remains historical and does not verify the corrected route. Corrections retain before/after values and field provenance in the latest 20 source events.

New CSV imports and provider results retain proposed values for differing nonempty fields. The contact detail view shows current and proposed values and offers **Accept source values** or **Keep current values**, with a required review reason. Decisions are recorded and stale/repeated reviews are rejected. Use the correction form when only some proposed changes should be applied, then explicitly record the source decision as appropriate. Older events lacking proposed values need a reviewed correction or a fresh source import. Identity collisions still require reviewing both records; this workflow deliberately does not merge people automatically. Rejected import identity conflicts remain in import reports and can be retried after correcting the underlying record/source.

Validation uses synthetic database and browser fixtures until a real licensed sample is supplied. This is not a claim of live provider accuracy or completed production deployment.
