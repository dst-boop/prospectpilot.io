...
const clean=v=>typeof v==='string'?v.slice(0,250):'';
...
export function professionalRecord(raw){
 ...
 return {provider_id:clean(raw.id),first_name:clean(raw.first_name),last_name:clean(raw.last_name),title:clean(raw.job_title),company:clean(raw.job_company_name),company_domain:clean(raw.job_company_website),industry:clean(raw.job_company_industry),seniority:Array.isArray(raw.job_title_levels)?raw.job_title_levels.map(clean).join(', '):'',country:clean(raw.location_country),state:clean(raw.location_region),city:clean(raw.location_locality),linkedin_url:normalizeProfile(raw.linkedin_url),email:emailAddress(raw.work_email),phone:(Array.isArray(raw.phone_numbers)?raw.phone_numbers:[]).map(phoneNumber).find(Boolean)||'',source:'People Data Labs',source_kind:'provider',email_status:emailAddress(raw.work_email)?'unverified':'missing',phone_status:(Array.isArray(raw.phone_numbers)?raw.phone_numbers:[]).some(phoneNumber)?'unverified':'missing'};
}
...
export function createProspectProviders({pdlKey='',hunterKey='',fetcher=fetch,now=()=>new Date()}={}){
 ...
 async function search(input={}){
  ...
  for(const [key,field] of Object.entries(mapping)){let value=clean(input[key]).trim().toLowerCase();if(!value)continue;if(key==='country'&&['us','usa'].includes(value))value='united states';must.push({match:{[field]:{query:value,operator:'and'}}});}
  ...
  const body={size,query:{bool:{must}},data_include:fields,...(input.scroll_token?{scroll_token:scrollToken(input.scroll_token)}:{})};
  ...
  if(!Array.isArray(response.data)||!Number.isFinite(response.total)||response.data.length>size)throw fail(502,'Provider returned an invalid search response.');
  ...
  return {contacts,total:response.total,scroll_token:scrollToken(response.scroll_token),retrieved:response.data.length,errors,provider:'pdl'};
 }
 async function enrich(contact){
  ...
  if(!response.data||Number(response.likelihood)<8)throw fail(502,'Provider identity confidence is insufficient.');
  ...
  return {contact:candidate,provider:'pdl',checked_at:now().toISOString()};
 }
 async function verifyEmail(contact){
  ...
  const status=data.accept_all===true?'catch_all':({valid:'valid',invalid:'invalid',accept_all:'catch_all',unknown:'unknown',webmail:'unknown',disposable:'invalid'})[data.status];
