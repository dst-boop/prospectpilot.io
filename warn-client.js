...
async function load(){
 ...
 W('coverage').replaceChildren();for(const c of d.coverage){const row=line(W('coverage'),c.state+': '+c.status.replaceAll('_',' ')+(c.status==='loaded'?' ('+c.rows+' rows; '+(c.unknown_notice_dates??0)+' undated; latest notice '+(c.latest_notice_date||'unknown')+')':''));if(c.missing_fields?.length)row.title='Fields not supplied: '+c.missing_fields.join(', ');if(c.status==='loaded')line(row,'Source updated: '+(c.published_at?new Date(c.published_at).toLocaleDateString():'unknown')+(c.publication_stale?' — over 48 hours old':''),'small');if(c.malformed_rows||c.excluded_non_warn)line(row,(c.malformed_rows||0)+' malformed rows and '+(c.excluded_non_warn||0)+' explicit non-WARN rows excluded.','small');}
