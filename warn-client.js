...
async function load(){
 ...
 W('freshness').textContent='Feeds retrieved '+new Date(d.fetched_at).toLocaleString()+'. Mirror published: '+(d.published_at?new Date(d.published_at).toLocaleString():'unknown')+(d.publication_stale?' — publication is over 48 hours old':'')+'. Individual source update dates appear below; notice dates remain separate.';
 W('coverage').replaceChildren();for(const c of d.coverage){const row=line(W('coverage'),c.state+': '+c.status.replaceAll('_',' ')+(c.status==='loaded'?' ('+c.rows+' rows; '+(c.unknown_notice_dates??0)+' undated; latest notice '+(c.latest_notice_date||'unknown')+')':''));if(c.missing_fields?.length)row.title='Fields not supplied: '+c.missing_fields.join(', ');if(c.status==='loaded')line(row,(c.publication_basis==='official_dataset'?'Dataset updated: ':'Mirror published: ')+(c.published_at?new Date(c.published_at).toLocaleDateString():'unknown')+(c.publication_stale?' — over 48 hours old':''),'small');if(c.date_anomalies)line(row,c.date_anomalies+' notices have implausible dates requiring review.','small');if(c.schema_notes?.length)line(row,c.schema_notes.join(' '),'small');if(c.malformed_rows||c.excluded_non_warn)line(row,(c.malformed_rows||0)+' malformed rows and '+(c.excluded_non_warn||0)+' explicit non-WARN rows excluded.','small');}
 ...
 for(const e of d.events){const article=document.createElement('article');line(article,e.employer,'h3');line(article,[e.city||e.address,e.county,e.state].filter(Boolean).join(' · '));line(article,'Notice: '+(e.notice_date||e.notice_date_note||'unknown')+' · Effective: '+(e.effective_date||e.date_note||'unknown'));if(e.quality_flags?.length)line(article,'Review required: implausible source date. Raw date text is retained above; it is excluded from date sorting and filtering.','small');line(article,'Workers: '+(e.workers??'unknown')+(e.reason?' · '+e.reason:''));const link=document.createElement('a');link.textContent='Published source';link.href=e.source_url;link.target='_blank';link.rel='noopener noreferrer';article.append(link);W('notices').append(article);}
